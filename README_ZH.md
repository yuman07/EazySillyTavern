...
- **完全本机** — SillyTavern 强制绑定 `127.0.0.1`，端口由 OS 在启动时随机分配。**永远不会暴露给局域网**。
...
| 资源 | 来源 | 约束 |
| --- | --- | --- |
| 临时端口 | `net.createServer().listen(0, '127.0.0.1')` | OS 选任意空闲端口；选完立即 close。 |
...
| 资源 | 来源 | 约束 |
| --- | --- | --- |
| 候选端口 | OS 通过 `listen(0)` 分配 | 分配时一定空闲；不限定端口段，kernel 给什么用什么。 |
...
- **为什么不限定端口段** — IANA 动态端口段（49152–65535）只是卫生习惯，不是正确性约束：服务只对 `127.0.0.1` 监听、每次随机重选，低位端口在功能上等价。少数 Windows 机器（自定义 dynamic port range、winsock 过滤器等）会持续给段外端口，强制段内会让启动器无谓地拒服务。
- **为什么仅 `service_crashed` 重试** — `fork_failed` 或 `missing_bundle`（Node 二进制或 SillyTavern 源码缺失）不是端口问题，重试只会延迟报错。
- **为什么 3 次** — 实测表明 `close()` 到 `listen()` 之间的 TOCTOU 命中率远低于 1%。三次独立重试连失败的概率压到 `1e-6` 以下，对于桌面启动器"足够好"。
