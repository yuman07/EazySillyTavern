...
| Node runtime 来源 | **随包附带 Node 22 LTS 官方二进制**（放 `extraResources/node/`）。Electron 自带的 Node 用 BoringSSL，缺 SHA-3 / SHAKE 系列哈希算法（`crypto.createHash('shake256')` 抛 `Error: Digest method not supported`），无法直接跑 SillyTavern 的 webpack 缓存版本计算。CI 在每个平台上下载对应 Node 二进制并装入安装包。本字段为对原 SPEC 假设的修正，实测于 SillyTavern 1.18.0 + Electron 38 |
...
3. Fork 一个子进程运行 SillyTavern 的入口脚本（即仓库内的 `server.js`），通过 CLI 参数透传以下配置（基于对 SillyTavern 1.18.0 `src/command-line.js` 的核对，全部为原生支持）：
   - `--port=<port>` 监听端口（步骤 1 选出的端口）
   - `--listen=false`、`--listenAddressIPv4=127.0.0.1`、`--enableIPv4=true`、`--enableIPv6=false` 监听地址强制 `127.0.0.1`
   - `--dataRoot=<userData>/data` 数据根目录（步骤 2 创建的标准用户目录，非 global 模式）
   - `--configPath=<userData>/data/config.yaml` 配置文件路径（首次启动 SillyTavern 自身的 `addMissingConfigValues` 会从 `default/config.yaml` 创建，含 `enableUserAccounts: false`）
   - `--basicAuthMode=false` 关闭 basicAuth
   - `--whitelist=false` 关闭白名单（127.0.0.1 监听下白名单是冗余项）
   - `--browserLaunchEnabled=false` 阻止 SillyTavern 自己开浏览器（我们用 Electron webview 加载）
   - 环境变量 `SILLYTAVERN_ENABLEUSERACCOUNTS=false` 显式锁死 userAccounts（无 CLI flag，但 SillyTavern 的 `getConfigValue` 支持 `SILLYTAVERN_<UPPER_SNAKE>` 形式的环境变量覆盖任何 config 项，见 `src/util.js:38`）
...
子进程使用 `child_process.spawn(<bundledNode>, ['server.js', ...args], { cwd, env, stdio })` 启动 SillyTavern。`<bundledNode>` 是随包附带的 Node 22 LTS 官方二进制（`process.resourcesPath/node/node` 在 mac、`/node/node.exe` 在 win），不复用 Electron 自带的 Node。

> **为什么不 fork() 复用 Electron-as-Node**：调研阶段发现 Electron（38.x）使用 BoringSSL，`crypto` 模块缺 SHAKE 系列哈希。SillyTavern 1.18.0 的 `webpack.config.js` 在 `getWebpackCacheVersion()` 中使用 `createHash('shake256')`，`preSetupTasks()` 阶段就抛 `Error: Digest method not supported` 立即退出。`--openssl-legacy-provider` 在 Electron 上不识别。bundle 一份完整的官方 Node 是最干净的做法，体积代价 ~25MB（mac arm64）/ ~30MB（win x64），仍在 SPEC 验收"≤ 250MB"上限内。
...
| Node 22 LTS 二进制 | `extraResources/node/node`（mac）或 `extraResources/node/node.exe`（win） | 由 build 脚本下载平台对应 Node 官方包并解出二进制；Electron 自带 Node 缺 SHAKE 哈希（详见 §四 Node runtime 来源） |
...
- 下载平台对应的 Node 22 LTS 官方包（`https://nodejs.org/dist/...`），仅解压出 `node` 二进制放到 `extraResources/node/`，其它 npm/docs/include 一律丢弃。

> 体积预期：Mac dmg ≈ 175–225 MB，Win exe ≈ 160–200 MB。Electron 自身约 100MB，SillyTavern + node_modules 约 80MB，附带 Node 二进制约 25–30MB。
...
>
> **就绪时序（基于 1.18.0 验证）**：在 SillyTavern 的启动序列里，所有路由在 `server.listen()` 之前就在模块顶层注册完毕（`src/server-main.js:214` 的 `app.get('/')`、`src/server-main.js:242` 的 `express.static('public')` 等），`preSetupTasks()`（plugins / 内容迁移 / settings init / webpack 编译）完成后才开始 listen，listen 后 `postSetupTasks()` 仅做日志和心跳。也就是说：**只要 socket 接受连接，根路径就一定能 200 响应 index.html**——不存在"过早就绪"的窗口。`enableUserAccounts=false` 下 `shouldRedirectToLogin` 始终返回 false，根路径不会 302 跳到 `/login`。
...
最大的两处实现风险已在 SillyTavern 1.18.0 上完成代码核对：

1. **SillyTavern 的启动注入接口** ✅：CLI 与环境变量两条注入通道齐全。CLI 覆盖 `port / listen / listenAddressIPv4 / enableIPv4 / enableIPv6 / dataRoot / configPath / basicAuthMode / whitelist / browserLaunchEnabled`；环境变量 `SILLYTAVERN_<UPPER_SNAKE>` 覆盖任意 config key（`src/util.js:38`），用于锁死 `enableUserAccounts=false`。`config.yaml` 缺失时由 SillyTavern 自身从 `default/config.yaml` 复制（`src/config-init.js:164`），无需 EazySillyTavern 介入。
2. **服务就绪探测** ✅：根路由在模块顶层注册（`src/server-main.js:214`），`server.listen()` 后立即可服务。`enableUserAccounts=false` 下根路径直返 200 + index.html，不会 302。`GET /` 作为健康检查端点可靠。

---

> **本文档约定**：所有 `[待确认]` 项已在 SillyTavern 1.18.0 上完成代码核对并就地敲定，文中 `[待确认]` 标记已全部移除。后续 bump SillyTavern 版本时若注入点 / 路由布局发生变化，需重新核对并更新本文。
