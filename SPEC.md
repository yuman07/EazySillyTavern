...
| 服务端口 | 每次启动通过 `listen(0)` 让 OS 分配一个空闲端口，不限定端口段 |
...
| Node 24 LTS 二进制 | `extraResources/node/node`（mac）或 `extraResources/node/node.exe`（win） | 由 build 脚本下载平台对应 Node 官方包并解出二进制；Electron 自带 Node 缺 SHAKE 哈希（详见 §四 Node runtime 来源） |
...
- 下载平台对应的 Node 24 LTS 官方包（`https://nodejs.org/dist/...`），仅解压出 `node` 二进制放到 `extraResources/node/`，其它 npm/docs/include 一律丢弃。
...
1. 用 Node 的 net 模块向 OS 申请一个空闲端口：
   ...
   server.close()
2. 不对端口范围做约束，OS 给哪个端口就用哪个：服务只对 127.0.0.1 监听、每次随机重选，端口高低位在功能上等价
